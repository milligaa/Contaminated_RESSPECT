...
def effective_fibre_mag(sep, gal_ddlr, sersic_index, galmag, pix_size_arcsec, seeing):
    ...
    PIL_img = Image.fromarray(true_array_int)
...
def template_flow(fibre_corr):

    #extracts data from the selfie files and returns the table with everything I need
    # data_to_use = SELFIE_extractor()[50:100]

    #uses the SELFIE file to get parameters like redshift, magnitude and SNe type for use in template generation
    template_values = adj_setup(Table.read('/Users/andrew/Desktop/Python_Stuff/SN_and_Galaxy/RESSPECT/Spectra_120224/params.csv', format='csv', delimiter=','))
    ...
    for dummy in range(len(Gmags)): 
        ...
        # try:
            ...
            result_observed = Comb_Maker(supernovae[dummy], galaxies[dummy],
            Gmags[dummy], Smags[dummy], SN_type_str[dummy], texp_visit[dummy], seeing_val,
            '/Users/andrew/Desktop/Python_Stuff/SN_and_Galaxy/RESSPECT/spectra_out/')
    ...
    ascii.write(SNR_table, '/Users/andrew/Desktop/Python_Stuff/SN_and_Galaxy/RESSPECT/results_out/SNR_res_120224.csv', format='csv', overwrite = True)
